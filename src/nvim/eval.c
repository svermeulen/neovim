...
static void api_wrapper(typval_T *argvars, typval_T *rettv, FunPtr fptr)
{
  ...
  if (ERROR_SET(&err)) {
    emsgf_multiline("EXXX: %s", err.msg);
    goto end;
  }
...
static void f_rpcrequest(typval_T *argvars, typval_T *rettv, FunPtr fptr)
{
  ...
  if (ERROR_SET(&err)) {
    ...
    if (name) {
      emsgf_multiline("Error when invoking '%s' on channel %"PRIu64" (%s):\n%s", method, chan_id, name, err.msg);
    } else {
      emsgf_multiline("Error when invoking '%s' on channel %"PRIu64":\n%s", method, chan_id, err.msg);
    }
